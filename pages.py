...
/* Hex Grid Canvas (Dark mode only) */
.hex-grid{position:fixed;inset:0;z-index:0;opacity:0;pointer-events:none;transition:opacity .5s ease}
[data-theme="dark"] .hex-grid{opacity:.7}
.hex-canvas{width:100%;height:100%;display:block}

/* Dark mode card enhanced glow */
[data-theme="dark"] .login-card{box-shadow:0 16px 48px rgba(0,0,0,.5), 0 0 80px rgba(43,127,255,.15), 0 0 160px rgba(123,97,255,.08);border-color:rgba(123,97,255,.15)}
[data-theme="dark"] .login-card::before{background:radial-gradient(circle,rgba(43,127,255,.12),transparent 70%)}
[data-theme="dark"] .login-card::after{background:radial-gradient(circle,rgba(123,97,255,.12),transparent 70%)}

/* Theme Toggle */
.theme-toggle{position:fixed;top:20px;left:20px;z-index:100;width:44px;height:44px;border-radius:50%;background:var(--surface);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:18px;color:var(--text-secondary);transition:all .3s ease;box-shadow:var(--shadow-md)}
.theme-toggle:hover{transform:scale(1.1);background:rgba(43,127,255,.1);color:var(--spider-blue);border-color:rgba(43,127,255,.2)}
[data-theme="dark"] .theme-toggle{background:rgba(15,23,42,.8);box-shadow:var(--shadow-md),0 0 20px rgba(43,127,255,.1)}

/* Version badge */
.version-badge{position:fixed;bottom:20px;left:20px;z-index:100;background:var(--surface);backdrop-filter:blur(12px);border:1px solid var(--border);padding:8px 14px;border-radius:30px;font-size:11px;font-weight:600;color:var(--text-secondary);box-shadow:var(--shadow-md)}
.version-badge span{color:var(--spider-blue)}

/* Responsive additions */
@media(max-width:480px){.theme-toggle{top:12px;left:12px;width:40px;height:40px}.version-badge{bottom:12px;left:12px;padding:6px 12px}}
</style>
</head>
...
<canvas class="hex-grid" id="hexGrid"></canvas>
<div class="particles">
  <div class="particle"></div><div class="particle"></div><div class="particle"></div>
  <div class="particle"></div><div class="particle"></div><div class="particle"></div>
  <div class="particle"></div><div class="particle"></div>
</div>
<button class="theme-toggle" id="themeToggle" title="تغییر تم" aria-label="Toggle theme">☀️</button>
<div class="version-badge">v4.0 <span>Spider Panel</span></div>
...
(function(){
  ...
  var hexSize=30;var animationId=null;var time=0;
  function resizeCanvas(){canvas.width=window.innerWidth;canvas.height=window.innerHeight}
  resizeCanvas();window.addEventListener('resize',resizeCanvas);
  function drawHex(x,y,size){ctx.beginPath();for(var i=0;i<6;i++){var angle=(Math.PI/3)*i-Math.PI/6;ctx.lineTo(x+size*Math.cos(angle),y+size*Math.sin(angle))}ctx.closePath();ctx.stroke()}
  function animate(){time+=0.003;ctx.clearRect(0,0,canvas.width,canvas.height);ctx.strokeStyle='rgba(43,127,255,.08)';ctx.lineWidth=0.8;var cols=Math.ceil(canvas.width/(hexSize*1.5))+1;var rows=Math.ceil(canvas.height/(hexSize*0.866))+1;for(var row=0;row<rows;row++){for(var col=0;col<cols;col++){var x=col*hexSize*1.5;var y=row*hexSize*0.866*2+(col%2)*hexSize*0.866;var pulse=Math.sin(time+col*0.5+row*0.3)*0.5+0.5;ctx.globalAlpha=0.15+pulse*0.15;drawHex(x,y,hexSize)}}ctx.globalAlpha=1;animationId=requestAnimationFrame(animate)}
  function startHexGrid(){if(animationId)cancelAnimationFrame(animationId);animate()}
  function stopHexGrid(){if(animationId)cancelAnimationFrame(animationId);ctx.clearRect(0,0,canvas.width,canvas.height)}
  var toggle=document.getElementById('themeToggle');
  toggle.onclick=function(){var cur=d.getAttribute('data-theme')||'light';var nxt=cur==='dark'?'light':'dark';d.setAttribute('data-theme',nxt);localStorage.setItem('spider-theme',nxt);updateThemeIcon(nxt);if(nxt==='dark')startHexGrid();else stopHexGrid()};
  function updateThemeIcon(theme){toggle.textContent=theme==='dark'?'🌙':'☀️'}
  if(saved==='dark')startHexGrid();
  var pw=document.getElementById('pw'),eye=document.getElementById('eye-btn'),eyeIcon=eye.querySelector('i');
  eye.onclick=function(){var show=pw.type==='password';pw.type=show?'text':'password';eyeIcon.className='ti '+(show?'ti-eye-off':'ti-eye')};
  document.getElementById('form').addEventListener('submit',async function(e){
    e.preventDefault();var btn=document.getElementById('btn'),err=document.getElementById('err'),et=document.getElementById('err-text');
    err.classList.remove('show');btn.disabled=true;btn.classList.add('btn-loading');
    try{var r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pw.value})});
    if(!r.ok){var d=await r.json().catch(function(){return{}});throw new Error(d.detail||'خطا در ورود')}
    location.href='/dashboard'}catch(ex){et.textContent=ex.message;err.classList.add('show');btn.disabled=false;btn.classList.remove('btn-loading')}});
})();
</script>
</body></html>"""
